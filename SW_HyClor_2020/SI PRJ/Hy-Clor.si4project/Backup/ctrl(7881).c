...
unsigned char bStopOutputFlag    = _FALSE;
...
{	
	ucData[11] = ucRunningMins;
	ucSaveType |= _RUNNING_DATA;		
}

void SaveCellRunningTime(void)
{	
	ucData[12] = ucRunningHours[0];
	ucData[13] = ucRunningHours[1];
	ucData[14] = ucRunningHours[2];
	ucData[15] = ucRunningHours[3];
	ucData[16] = ucRunningHours[4];
	ucData[17] = ucRunningHours[5];
	ucSaveType |= _RUNNING_DATA;		
}
#endif
...
unsigned char DataRangeCheck(unsigned int uiAddr)
{
	...
	if(ucData[10] == 255)
		return 1;
#else
	if((ucData[9] > 252)||(ucData[9] < 100))
		return 1;
	if((ucData[10] > 252)||(ucData[10] < 100))
		return 1;
#endif	
	return 0;
}
...
void SetToDefault(void)
{
	...
	//if((ucData[2] == MODEL_CSM15)||(ucData[2] == MODEL_CSM25))
	//	ucData[5] = TIME_10HOURS;
	//else
		ucData[5] = TIME_7P5HOURS;
	ucData[6] = 0;
	ucData[7] = 0;
	ucData[8] = 0;
	ucData[9] = 70;
...
{
	ucRunningMins = ucData[11];
	ucRunningHours[0] = ucData[12];
	ucRunningHours[1] = ucData[13];
	ucRunningHours[2] = ucData[14];
	ucRunningHours[3] = ucData[15];
	ucRunningHours[4] = ucData[16];
	ucRunningHours[5] = ucData[17];
}
...
void EepromDataCheck(void)
{
	...
	if(bDataErrorFlag == 1)
	{
		...
		{
			SaveDataToArea(EEPROM_START_ADDRESS,11,&ucData[0]);
			bNeedResetFlag = 0;
		}
	}
	else
	...
	for(i=0;i<7;i++)
	{	  
		ucData[11+i] = Eeprom_ReadBytes(k);
		k++;
	}
	if((ucData[11] > 60)||(ucData[12] > 9)||(ucData[13] > 9)||(ucData[14] > 9)||(ucData[15] > 9)||(ucData[16] > 9)||(ucData[17] > 9))
	{
		ClearRunningData();
		SaveDataToArea(CELL_RUNNING_MINS_ADDR,7,&ucData[11]);
	}	
	UpdateRunningData();
#endif	
	...
	if(bNeedResetFlag == 1)
	{
		...
		SetToDefault();
		SaveDataToArea(EEPROM_START_ADDRESS,11,&ucData[0]);
		SaveDataToArea(DATA_BACKUP_ADDR,11,&ucData[0]);
#if(SW_VERSION_DISPLAY	==_ENABLE)        
		SWVersionDisplay();
#endif    
	}
...
#else
	uiTemp = 805664;
	uiTemp2 = 625 - ucCurrentCalValue;
	uiTemp = uiTemp/uiTemp2;
#endif
	ucConvertGain[0] = uiTemp/10;
	uiTemp2 = 10;
...
void CellOutputCurrentAdjust(unsigned char ucAdjDirect)
{
...
#endif
	{
		...
		//uiOutputTime = 12*ucCellCurrent;
		iTemp = ucCellCurrent;
		ucNum = 10;
		ucNum = iTemp/ucNum;
		uiOutputTime = 24*ucNum;
...
void CellOutputControl(void)
{
	...
	uiOutputTime = 96;// 12*ucCellCurrent;
	CellCurrentOutput(ucCellCurrent);
	CalculateThresholdLimit();

	Delay_ms(200);
	
	ucDisplayWaitTimerCnt = 3;	//Waiting 3 Second than start display the current
}
...
#endif
	/*
	if((ucCellCurrent>0)&&(uiCellRealCurrent == 0))
	{
		ucWaterFlowCounter++;
		if(ucWaterFlowCounter >= 5)
		{
			if(bNoWaterFlag == _FALSE)
			{
				bNoWaterFlag = _TRUE;
				LEDFlashSetting(LED_13,_ON);
			}
		}
	}
	else
	{
		if(bNoWaterFlag == _TRUE)
		{
			LEDFlashSetting(LED_13,_OFF);
			CellCurrentOutput(ucCellCurrent);
			bNoWaterFlag = _FALSE;
		}
...
#else
	uiTemp += 50;
	ucGainValue =(unsigned char)(uiTemp/100);
	if(ucGainValue>=625)
		ucGainValue = 0;
	else
		ucGainValue = 625- ucGainValue;
#endif
}
#endif
...
{	
	...
	SaveEventProcess();
	//if((ucBoostStatus==0)&&(bDoBoostModeFlag==0))
	if(ucBoostStatus==0)
	{
		GetCellRealCurrent();
		//if((bCalibrationFlag)&&(bADCUpdateFlag))
		//if(ucDisplayWaitTimerCnt > 0)
		//{
